...
namespace RaspHelloWord
{
    class PowerMeter
    {
        ...
        public PowerMeter(string System, int port)
        {
            try
            {
        ...
        public void sendBadRequest( byte[] data)
        {
            byte[] send = new byte[data.Length];

            Buffer.BlockCopy(data , 0, send, 0, data.Length);
            _ns.Write(send, 0, send.Length);
            Console.WriteLine("Inviato:" + BitConverter.ToString(send));

        }
        public byte[] sendRequest(byte IdDevice, byte Command, ushort StartAddr, ushort Howmany)
        {
        ...
        private byte[] _readModbusMsg()
        {
            ...
            //
            Console.WriteLine("Ricevuto:" + BitConverter.ToString(fromdevice));
            return fromdevice;
        }
